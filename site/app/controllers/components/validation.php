        ...
        if (!empty($result)) {
            foreach ($result as $locale) {
                ...
                if (property_exists($info, 'unmodifiedEntities')) {
                    $results[] = $this->_result(TEST_WARN, 0, '', sprintf(___('devcp_error_addon_translations_unmodified', 'The %s locale contains %d unmodified translations'), $code, $info->unmodifiedEntities));
                }
                if (property_exists($info, 'missingEntities')) {
                    $results[] = $this->_result(TEST_WARN, 0, '', sprintf(___('devcp_error_addon_translations_missing', 'The %s locale is missing %d translations'), $code, $info->missingEntities));
                }
            }
        }
        return $this->_passIfEmpty($results);
