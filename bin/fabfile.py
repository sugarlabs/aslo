...
def create_dirs():
    local("mkdir -p %s"% TMP)
    local("mkdir -p %s" % os.path.join(SPHINX_DIR, 'data/sphinx'))
    local("mkdir -p %s" % os.path.join(SPHINX_DIR, 'log/searchd"))
...
def install_sphinx():
    ...
    #make
    local("cd %s;./configure --prefix=%s;make;make install"%(os.path.join(TMP, SPHINX_VERSION),ROOT))
